  ...
  <ModalDialog :model-value="modelValue" title="Cómo clasificar un concepto" size="2xl" @update:model-value="emit('update:modelValue', $event)">
    <div class="space-y-5 text-sm text-pic-text-main">
      <ol class="grid gap-2 sm:grid-cols-2">
        <li v-for="(step, index) in ['Selecciona y reserva un concepto', 'Aplica o revisa la propuesta', 'Corrige y limpia los campos necesarios', 'Confirma la inserción en ArticulosIC']" :key="step" class="flex gap-3 rounded-lg border border-pic-border bg-pic-muted-surface p-3">
          <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-pic-brand text-[10px] font-bold text-white">{{ index + 1 }}</span>
      ...
      <section class="rounded-lg border border-[hsl(var(--pic-info)/0.24)] bg-[hsl(var(--pic-info)/0.07)] p-3">
        <h3 class="text-[10px] font-bold uppercase tracking-[0.14em] text-pic-info">Aplicar no es guardar</h3>
        <p class="mt-1 text-xs font-semibold leading-5 text-pic-text-muted">
          <strong class="text-pic-text-main">Aplicar propuesta</strong> solo llena el formulario. Únicamente <strong class="text-pic-text-main">Confirmar e insertar</strong> crea la fila en ArticulosIC.
        </p>
      </section>
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        ...
        <section>
          ...
          <ul class="mt-2 space-y-2 text-xs font-semibold leading-5 text-pic-text-muted">
            ...
            <li><i class="fa-solid fa-coins mr-2 w-3 text-pic-warning"></i>Generar propuestas llama OpenAI y consume créditos.</li>
      ...
      <p class="border-t border-pic-border pt-3 text-[10px] font-semibold leading-4 text-pic-text-muted">
        Si catálogos, antecedentes u OpenAI no están disponibles, puedes continuar manualmente. Posponer requiere un motivo y no inserta el artículo.
      </p>
    </div>
    <template #footer>
