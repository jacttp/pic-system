<!-- src/modules/StockAnalytics/components/StockInventoryWeeklyTable.vue -->
...
const filtered = computed(() => {
    ...
    if (!q) return store.groupedInventorySummary;
    return store.groupedInventorySummary.filter(g =>
...
function rowAvg(row: Record<string, string | number>): number {
    const weeks = store.inventoryWeekColumns;
    if (!weeks.length) return 0;
    const sum = weeks.reduce((acc, w) => acc + (Number(row[String(w)]) || 0), 0);
    return sum / weeks.length;
}

function clientWeekSum(skus: typeof store.inventorySummary, week: number): number {
    return skus.reduce((acc, sku) => acc + (Number(sku[String(week)]) || 0), 0);
}

function clientAvg(skus: typeof store.inventorySummary): number {
    const weeks = store.inventoryWeekColumns;
    if (!weeks.length) return 0;
...
const maxWeekValue = computed((): number => {
    ...
    for (const group of filtered.value) {
        for (const sku of group.skus) {
            for (const w of store.inventoryWeekColumns) {
...
function heatClass(value: number): string {
    ...
    if (ratio < 0.15) return 'bg-sky-50 text-sky-700';
    if (ratio < 0.35) return 'bg-sky-100 text-sky-800';
    if (ratio < 0.55) return 'bg-sky-200 text-sky-900';
    if (ratio < 0.75) return 'bg-sky-300 text-sky-900 font-medium';
    ...
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        ...
        <div class="flex items-center justify-between px-5 py-3 border-b border-slate-100 bg-slate-50">
            <h3 class="text-sm font-semibold text-slate-700">
                <i class="fa-solid fa-warehouse mr-2 text-sky-500"></i>
                Sábana Semanal — Inventario
            ...
            <input
                ...
                class="h-8 w-56 text-xs rounded-lg border border-slate-200 bg-white px-3
                       ...
                       focus:outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-400/20 transition"
            />
        </div>
            ...
            <table class="w-full text-xs min-w-max border-collapse">
                ...
                <thead>
                    <tr class="bg-slate-800 text-slate-200">
                        ...
                        <th
                            v-for="w in store.inventoryWeekColumns"
                            :key="w"
                ...
                <tbody>
                    ...
                    <template v-if="store.isLoadingData">
                        <tr v-for="i in 6" :key="i" class="border-b border-slate-100">
                            ...
                            <td :colspan="(store.inventoryWeekColumns.length || 6) + 2">
                    ...
                    <template v-else-if="!filtered.length">
                        <tr>
                            <td :colspan="store.inventoryWeekColumns.length + 3" class="text-center py-12 text-slate-400">
                                Sin registros.
                            </td>
                        </tr>
                    </template>
