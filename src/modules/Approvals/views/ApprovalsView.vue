...
// Cualquier usuario puede ser aprobador si tiene solicitudes asignadas.
// Level 4 siempre ve la tab "assigned" poblada (ve todas).
const userLevel  = computed(() => authStore.user?.accessLevel ?? 1);
...
const activePending = computed(() =>
   activeList.value.filter(a => a.status === 'PENDING')
);
...
// En tab "assigned" el usuario puede resolver; en "mine" no (es el solicitante)
...
const handleFilter = (filters: ApprovalFilters) => {
   if (activeTab.value === 'assigned') {
      approvalsStore.fetchAssignedApprovals(filters);
   } else {
...
const handleView = (id: number) => {
   selectedApproval.value = activeList.value.find(a => a.id === id) || null;
...
const handleCancel = async (id: number) => {
   ...
   } catch { /* error manejado por store */ }
};
</script>
...
<template>
   <div class="p-6 lg:p-8 max-w-[1200px] mx-auto">
      ...
      <div class="mb-6 flex items-start justify-between">
         ...
         <!-- Badge total pendientes -->
         <div v-if="approvalsStore.totalPendingCount > 0"
              class="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2 text-center">
            <div class="text-2xl font-bold text-amber-600">{{ approvalsStore.totalPendingCount }}</div>
      ...
      <div class="flex gap-2 mb-6 border-b border-slate-200">
         ...
         >
            ...
            Por resolver
            <span
               v-if="approvalsStore.assignedPendingCount > 0"
               class="bg-amber-100 text-amber-700 text-[10px] font-bold px-1.5 py-0.5 rounded-full"
            >
         ...
         >
            ...
            <span
               ...
               class="bg-slate-200 text-slate-600 text-[10px] font-bold px-1.5 py-0.5 rounded-full"
            >
      ...
      <!-- Loading skeleton -->
      <div v-if="isActiveLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <!-- Error -->
      <div v-else-if="approvalsStore.error"
         ...
         <button @click="handleResolved" class="mt-3 text-xs text-red-600 hover:underline">
            Reintentar
         </button>
      </div>
      ...
      <template v-else>
         ...
         <!-- Sección: Pendientes -->
         <div v-if="activePending.length > 0" class="mb-8">
            <h2 class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-2">
               <span class="w-2 h-2 rounded-full bg-amber-400"></span>
               Pendientes ({{ activePending.length }})
            </h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
               <ApprovalCard
                  v-for="approval in activePending"
                  :key="approval.id"
                  :approval="approval"
                  :can-resolve="canResolveInTab"
                  @view="handleView"
         ...
         <!-- Sección: Historial -->
         <div v-if="activeResolved.length > 0">
            <h2 class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-2">
               <span class="w-2 h-2 rounded-full bg-slate-300"></span>
               Historial ({{ activeResolved.length }})
            </h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
               <ApprovalCard
                  v-for="approval in activeResolved"
                  :key="approval.id"
                  :approval="approval"
                  :can-resolve="false"
                  @view="handleView"
