...
aria - hidden="true"
   ...
   < !--Filters Content con transición suave-- >
      <Transition
      name="collapse"
@enter="(el: HTMLElement) => { el.style.height = el.scrollHeight + 'px' }"
@after-enter="(el: HTMLElement) => { el.style.height = '' }"
@leave="(el: HTMLElement) => { el.style.height = el.scrollHeight + 'px'; requestAnimationFrame(() => { el.style.height = '0' }) }"
   >
   <div v -if= "!isCollapsed" id = "filters-content" class="p-6 space-y-6" >
